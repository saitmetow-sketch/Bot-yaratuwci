...
async def my_bots_handler(message: types.Message):
    ...
    for b in bots:
        remaining = (b["expire_at"] - datetime.now()).days
        status = "🟢 Active" if remaining > 0 else "🔴 Muddati tugagan"
        remaining_text = max(remaining, 0)
        text += f"@{b['bot_username']}\n⏳ Qolgan: {remaining_text} kun — {status}\n\n"
...
async def create_bot_menu_handler(message: types.Message, state: FSMContext):
    await message.answer(
        ...
        "⚠️ Tokenni hech kimga ko'rsatmang, faqat shu yerga yuboring."
    )
    await state.set_state(AdminStates.waiting_bot_token)


@dp.message(AdminStates.waiting_bot_token)
async def receive_bot_token(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer("❌ Token noto'g'ri. Qaytadan tekshirib, to'g'ri tokenni yuboring.")
        return

    await db.add_created_bot(
        bot_id=me.id,
    ...
    await message.answer(
        ...
        f"🆔 Bot ID: {me.id}\n\n"
        f"⏳ 3 kun bepul ishlaydi. Botni real ishga tushirish (kino qidirish va h.k.) keyingi bosqichda qo'shiladi."
    )
    await state.clear()
